...
# - Start menu saldo awal (UI menarik) ditampilkan duluan sebelum toko
# - Pada kartu katalog, dekat harga ditambahkan tombol + dan − untuk menambah/menghapus jumlah langsung
# - Setelah klik "Tambah ke Keranjang": dialog dengan gambar, nama, harga, dan opsi "Lanjut Belanja" atau "Lihat Keranjang Saya"
# =========================================
...
def show_colored_dialog(root, title, message, bg="#1b1b2e", fg_title="#FFD700", fg_msg="white"):
    ...
    win.configure(bg=bg)
    win.grab_set()
    tk.Label(win, text=title, font=font(size=18, weight="bold"), bg=bg, fg=fg_title).pack(pady=14)
    tk.Label(win, text=message, font=font(size=12), bg=bg, fg=fg_msg, wraplength=500, justify="center").pack(pady=10)
...
class StoreApp:
    def __init__(self, root):
        ...
        # State awal
        # Perubahan: saldo default 0, diminta via start menu
        self.balance = 0
        self.cart = Cart()
        ...
        self.voucher_banner_var = tk.StringVar(value="")  # banner voucher
        ...
        # =========================
        # Siapkan empat menu (frame): Start (Saldo), Store, Keranjang, Detail
        # =========================
        self.start_frame = tk.Frame(root, bg="#101020")  # Start menu saldo (run duluan)
        ...
        self.build_start_menu()    # tambahan sesuai permintaan
        self.build_store()
        self.build_cart()
    ...
    # =========================
    # Start menu Saldo Awal (UI menarik, tampil duluan)
    # =========================
    def build_start_menu(self):
        header = tk.Frame(self.start_frame, bg="#101020")
        header.pack(fill="x")
        tk.Label(header, text="🪙 Selamat Datang di Game Store", font=font(size=26, weight="bold"), bg="#101020", fg="#FFD700").pack(pady=20)

        card = tk.Frame(self.start_frame, bg="#1b1b2e", bd=2, relief="ridge")
        card.pack(pady=20)
        tk.Label(card, text="Atur Saldo Awal Kamu", font=font(size=18, weight="bold"), bg="#1b1b2e", fg="#32cd32").pack(pady=12)
        tk.Label(card, text="Masukkan angka tanpa titik/koma", font=font(size=11), bg="#1b1b2e", fg="#a0a0c0").pack()

        entry_wrap = tk.Frame(card, bg="#1b1b2e")
        entry_wrap.pack(pady=12)
        self.entry_saldo = tk.Entry(entry_wrap, font=font(size=18), width=14, justify="center", bg="#23233a", fg="white")
        self.entry_saldo.insert(0, "500000")
        self.entry_saldo.pack(side="left", padx=6)
        tk.Button(entry_wrap, text="Set Rp 1.000.000", command=lambda: self.entry_saldo.delete(0, "end") or self.entry_saldo.insert(0, "1000000"),
                  bg="#8a2be2", fg="white", font=font(size=12, weight="bold")).pack(side="left", padx=6)

        actions = tk.Frame(card, bg="#1b1b2e")
        actions.pack(pady=16)
        tk.Button(actions, text="Mulai Belanja 🎮", command=self.start_store, bg="#32cd32", fg="white", font=font(size=14, weight="bold"), width=20).pack(side="left", padx=8)
        tk.Button(actions, text="Keluar ❌", command=self.root.destroy, bg="#ff6347", fg="white", font=font(size=14, weight="bold"), width=12).pack(side="left", padx=8)
    ...
    def start_store(self):
        try:
            self.balance = max(0, int(self.entry_saldo.get()))
        except:
            self.balance = 500000
        self.show_store()
    ...
    def create_game_card(self, parent, game, index):
        """Kartu game menarik, ikon kecil 3 kolom, klik gambar ke menu detail, harga dengan coret jika diskon.
           Tambahan: tombol + / − di dekat harga, dan dialog konfirmasi saat tambah."""
        ...
        # Tambahan: tombol + / − dekat harga untuk atur jumlah langsung
        qty_row = tk.Frame(container, bg="#2f2f4f")
        qty_row.pack(pady=6)
        tk.Button(qty_row, text="+ Tambah", command=lambda g=game: self._catalog_inc(g), bg="#32cd32", fg="white", font=font(size=11, weight="bold")).pack(side="left", padx=4)
        tk.Button(qty_row, text="− Hapus", command=lambda g=game: self._catalog_dec(g), bg="#ff6347", fg="white", font=font(size=11, weight="bold")).pack(side="left", padx=4)

        # Tombol tambah ke keranjang (menampilkan dialog pilihan)
        tk.Button(container, text="Tambah ke Keranjang", command=lambda g=game: self.add_to_cart_with_prompt(g),
    ...
    def _render_price_labels(self, game, area):
        ...
        if harga_aktif < harga_asli:
            area["label_asli"].config(
                ...
                font=(font()[0], 12, "overstrike", "bold"),
                fg="#ff6347",
                bg="#2f2f4f"
            )
    ...
    # =========================
    # Aksi katalog tambahan (+/− di kartu) dan dialog tambah
    # =========================
    def _catalog_inc(self, game):
        ...
        self._show_added_menu(game, just_added=True)

    def _catalog_dec(self, game):
        current_price = self.effective_price(game)
        g_snapshot = Game(game.key, game.title, current_price, game.cover)
        self.cart.remove(g_snapshot)
        show_colored_dialog(self.root, "Dihapus", f"Mengurangi 1 {game.title} dari keranjang.", bg="#3b1f24", fg_title="#ffcc00", fg_msg="white")

    def add_to_cart_with_prompt(self, game):
        current_price = self.effective_price(game)
        g_snapshot = Game(game.key, game.title, current_price, game.cover)
        self.cart.add(g_snapshot)
        self._show_added_menu(game)

    def _show_added_menu(self, game, just_added=False):
        """Dialog konfirmasi setelah tambah item: tampilkan gambar, nama, harga, dan dua opsi."""
        win = tk.Toplevel(self.root)
        win.title("Item Ditambahkan")
        win.geometry("640x420")
        win.configure(bg="#1b1b2e")
        win.grab_set()

        tk.Label(win, text="✅ Ditambahkan ke Keranjang", font=font(size=20, weight="bold"), bg="#1b1b2e", fg="#32cd32").pack(pady=10)

        info = tk.Frame(win, bg="#23233a", bd=2, relief="ridge")
        info.pack(pady=10, padx=16, fill="x")
        img = None
        if os.path.exists(game.cover):
            try:
                img = self._load_image(game.cover, (220, 130))
            except:
                img = None
        if img:
            tk.Label(info, image=img, bg="#23233a").pack(side="left", padx=12, pady=10)
        else:
            tk.Label(info, text="[Gambar tidak tersedia]", font=font(size=11, weight="bold"), bg="#23233a", fg="#ff6b6b").pack(side="left", padx=12, pady=10)

        txt = tk.Frame(info, bg="#23233a"); txt.pack(side="left", padx=12, pady=10)
        tk.Label(txt, text=game.title, font=font(size=14, weight="bold"), bg="#23233a", fg="white").pack(anchor="w")
        tk.Label(txt, text=f"Harga: {format_rupiah(self.effective_price(game))}", font=font(size=12, weight="bold"), bg="#23233a", fg="#FFD700").pack(anchor="w", pady=2)
        tk.Label(txt, text=f"Jumlah di keranjang: {self.cart.qty_by_title(game.title)}", font=font(size=12), bg="#23233a", fg="#a0ffea").pack(anchor="w", pady=2)

        actions = tk.Frame(win, bg="#1b1b2e"); actions.pack(pady=16)
        tk.Button(actions, text="Lanjut Belanja 🛍️", command=win.destroy, bg="#32cd32", fg="white", font=font(size=12, weight="bold"), width=18).pack(side="left", padx=8)
        tk.Button(actions, text="Lihat Keranjang Saya 🛒", command=lambda: [win.destroy(), self.show_cart()], bg="#ffd700", fg="black", font=font(size=12, weight="bold"), width=22).pack(side="left", padx=8)
        ...
        for game, qty in self.cart.items.items():
            ...
            if os.path.exists(game.cover):
                try:
                    ...
                    tk.Label(left, text="[Img Err]", bg="#4f4f6f", fg="red", font=font(size=12, weight="bold")).pack()
            else:
                tk.Label(left, text="[Img Missing]", bg="#4f4f6f", fg="red", font=font(size=12, weight="bold")).pack()
    ...
    def build_detail_base(self):
        ...
        self.detail_title_lbl = tk.Label(self.detail_header, text="", font=font(size=22, weight="bold"), bg="#1b1b2e", fg="#FFD700")
        self.detail_title_lbl.pack(side="left", padx=22, pady=12)
        tk.Button(self.detail_header, text="⬅️ Back", command=self.show_store, bg="#1e90ff", fg="white", font=font(size=12, weight="bold")).pack(side="right", padx=10, pady=10)
    ...
    def show_detail(self, game):
        ...
        if not meta:
            tk.Label(self.detail_content_holder, text="Detail belum tersedia.", font=font(size=12, weight="bold"), bg="#1b1b2e", fg="#ffcc00").pack(pady=16)
            return
        ...
        if os.path.exists(game.cover):
            try:
                ...
                tk.Label(img_frame, text="[Gambar gagal dimuat]", font=font(size=12, weight="bold"), bg="#1b1b2e", fg="#ff6b6b").pack()
        else:
            tk.Label(img_frame, text="[Gambar tidak ditemukan]", font=font(size=12, weight="bold"), bg="#1b1b2e", fg="#ff6b6b").pack()

        # Tombol Trailer
        btns = tk.Frame(self.detail_content_holder, bg="#1b1b2e")
        btns.pack(pady=8)
        tk.Button(btns, text="▶ Putar Trailer", command=lambda: self.play_trailer(meta["trailer"]), bg="#8a2be2", fg="white", font=font(size=12, weight="bold")).pack()
        ...
        tk.Label(block, text="Deskripsi", font=font(size=16, weight="bold"), bg="#23233a", fg="#FFD700").pack(anchor="w")
        tk.Label(block, text=meta["desc"], font=font(size=12), bg="#23233a", fg="white", wraplength=1060, justify="left").pack(anchor="w", pady=4)

        # Bagian konten dewasa
        block2 = tk.Frame(inner, bg="#23233a")
        block2.pack(fill="x", padx=10, pady=8)
        tk.Label(block2, text="Deskripsi Konten Dewasa", font=font(size=16, weight="bold"), bg="#23233a", fg="#ff6b6b").pack(anchor="w")
        tk.Label(block2, text=meta["adult"], font=font(size=12), bg="#23233a", fg="#ffd7d7", wraplength=1060, justify="left").pack(anchor="w", pady=4)

        # Spesifikasi
        block3 = tk.Frame(inner, bg="#23233a")
        block3.pack(fill="x", padx=10, pady=8)
        tk.Label(block3, text="Persyaratan Sistem", font=font(size=16, weight="bold"), bg="#23233a", fg="#32cd32").pack(anchor="w")
        tk.Label(block3, text=meta["spec"], font=font(size=12), bg="#23233a", fg="white", justify="left").pack(anchor="w", pady=4)
        ...
        if devpub:
            ...
            tk.Label(block4, text=devpub, font=font(size=12, weight="bold"), bg="#23233a", fg="#a0ffea").pack(anchor="w")
    ...
    def play_trailer(self, path):
        ...
        if os.path.exists(path):
            try:
                ...
                show_colored_dialog(self.root, "Gagal Memutar Trailer", f"Trailer tidak dapat diputar.\n{e}", bg="#3b1f24", fg_title="#ff6b6b", fg_msg="#ffd7d7")
        else:
            show_colored_dialog(self.root, "Trailer Tidak Ditemukan", f"File trailer tidak ditemukan:\n{path}", bg="#2b2b44", fg_title="#ffcc00", fg_msg="white")

    # =========================
    # Interaksi Store <-> Cart, Checkout, Struk, Spin
    # =========================
    def show_store(self):
    ...
    def add_to_cart(self, game):
        ...
        g_snapshot = Game(game.key, game.title, current_price, game.cover)
        self.cart.add(g_snapshot)
        show_colored_dialog(self.root, "Ditambahkan", f"{game.title} ditambahkan ke keranjang.\nHarga: {format_rupiah(current_price)}", bg="#223b2b", fg_title="#a9ffaf", fg_msg="#dfffe2")
    ...
    def confirm_checkout_ui(self, total_pay, penerima_text):
        ...
        ok_btn = tk.Button(actions, text="Ya, Lanjutkan Pembelian ✅", bg="#32cd32", fg="white", font=font(size=12, weight="bold"), width=24, command=lambda: [setattr(win, "confirmed", True), win.destroy()])
        ok_btn.pack(side="left", padx=10)
        cancel_btn = tk.Button(actions, text="Batalkan ❌", bg="#ff6347", fg="white", font=font(size=12, weight="bold"), width=16, command=lambda: [setattr(win, "confirmed", False), win.destroy()])
    ...
    def show_receipt(self, purchased_summary, spin_count):
        ...
        for title, qty, subtotal, cover in purchased_summary:
            ...
            if os.path.exists(cover):
                try:
                    ...
                    tk.Label(row, text="[IMG Err]", font=font(size=12, weight="bold"), bg="#2e2e4a", fg="red").pack(side="left", padx=12, pady=10)
            else:
                tk.Label(row, text="[IMG Missing]", font=font(size=12, weight="bold"), bg="#2e2e4a", fg="red").pack(side="left", padx=12, pady=10)
        ...
        spin_btn = tk.Button(win, text="Mainkan Lucky Spin", bg="#8a2be2", fg="white", font=font(size=12, weight="bold"), command=lambda: self.open_simple_spin(win, spin_btn))
        spin_btn.pack()
        ...
        tk.Button(action_bar, text="Kembali ke Toko", command=lambda: [win.destroy(), self.show_store()], bg="#32cd32", fg="white", font=font(size=12, weight="bold"), width=16).pack(side="left", padx=8)
    ...
    def daily_deal_popup(self):
        ...
        if os.path.exists(rekom.cover):
            try:
                ...
                tk.Label(deal_win, text="[Gambar gagal dimuat]", bg="#1e1e2f", fg="red", font=font(size=12)).pack(pady=12)
        else:
    ...
    def _build_details(self):
        ...
        d["exp33"] = {
            ...
            "adult": "Kekerasan, darah, bunuh diri, imagery yang mengganggu.",
        ...
        d["gow"] = {
            ...
            "spec": "Minimum: i5-2500k, GTX 960 4GB, RAM 8 GB — Rekomendasi: i5-6600k GTX 1060 6GB, RAM 8GB.",
        ...
        d["p5r"] = {
            ...
            "spec": "Minimum: i7-4790, GTX-650 TI 2GB, RAM 8GB — Rekomendasi: i7-4790, GTX 760 2GB, RAM 8GB",
